...
def save_summary_files(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    consolidated_csv = out_dir / "mesonet_experimentos_consolidados.csv"
    ...
    lines = [
        "Resumo consolidado dos experimentos de imagens MesoNet",
        "=====================================================",
        ...
        f"- dropout_conv: {best.get('dropout_conv', '')}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Consolida todos os experiments/mesonet em um único CSV."
    )
    parser.add_argument(
        "--experiments-root",
        default="experiments/mesonet",
        help="Pasta raiz dos experimentos da MesoNet.",
    )
    parser.add_argument(
        "--out-dir",
        default="reports/mesonet_image/tables/consolidado",
