...
def evaluate_checkpoint(
    ...
    model = Meso4(num_classes=len(classes), dropout_conv=dropout_conv, dropout_fc=dropout_fc).to(device)
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Avaliar checkpoint da MesoNet no conjunto de teste.')
    parser.add_argument('--model-path', required=True)
    parser.add_argument('--split-dir', default='data/splits/split_01')
    parser.add_argument('--img-size', type=int, default=256)
    parser.add_argument('--batch-size', type=int, default=32)
    parser.add_argument('--num-workers', type=int, default=0)
    parser.add_argument('--pin-memory', action='store_true')
    parser.add_argument('--dropout-conv', type=float, default=0.25)
    parser.add_argument('--dropout-fc', type=float, default=0.0)
