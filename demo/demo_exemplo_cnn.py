...
MODEL_PATH = PROJECT_ROOT / "checkpoints" / "cnn" / "best_cnn_baseline_overall.pt"
...
OUT_PATH = PROJECT_ROOT / "reports" / "cnn_image" / "figures" / "demo" / "demo_real_fake.png"
...
def load_model(model_path: Path) -> CNNBaseline:
...
def predict_face(model: CNNBaseline, face_bgr: np.ndarray) -> tuple[str, np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    fig.suptitle(
    "Exemplos visuais de predição com o melhor modelo CNN Baseline",
    color=PURPLE,
    fontsize=16,
    fontweight="bold",
    )
